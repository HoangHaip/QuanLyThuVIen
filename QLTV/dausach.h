		...
		if(curMenu == btnQLDauSach.id){
			...
			if (curMenu == btnQLDauSach.id) {
			    if (edTimDauSach.isMouseHover(mx, my)) {
			        ...
			        DrawList(); // C?p nh?t danh sách khi ngu?i dùng nh?p t? khóa
			    }
			}
...
void TimKiemDauSach(char* keyword) {
    sizeListIndexDauSachSearch = 0;
    for (int i = 0; i < DSDS.n; ++i) {
        if (strstr(DSDS.nodes[i]->tensach, keyword) != NULL || strstr(DSDS.nodes[i]->tacgia, keyword) != NULL || strstr(DSDS.nodes[i]->theloai, keyword) != NULL) {
            listIndexDauSachSearch[sizeListIndexDauSachSearch++] = i;
        }
    }
}
...
void DrawList() {
    ...
    if (strlen(edTimDauSach.content) == 0) {
        ...
        TimKiemDauSach(edTimDauSach.content); // G?i hàm tìm ki?m
